...
def discrete_scatter(
    ...
    for i, (yy, color_ind) in enumerate(zip(unique_y, cr)):
        ...
        if np.mean(colorConverter.to_rgb(color)) < 0.2:
    ...
    if label_points:
        ...
        for i, txt in enumerate(labs):
            ...
            ax.annotate(
                txt,
                (x1[i], x2[i]),
                xytext=(x1[i] + x1_annot, x2[i] + x2_annot),
                c="k",
                size=font_size,
            )
...
def plot_pca_reconstructions(X, pca):
    ...
    fig = make_subplots(
        rows=1, cols=2, subplot_titles=("Original data", "Reconstructions (X_hat)")
    )
...
def plot_interactive_3d(X):
    trace = go.Scatter3d(
        ...
        z=X[:, 2],
        mode="markers",
        marker=dict(color="linen", size=8, line=dict(color="black", width=3)),
    )
    layout = go.Layout(
        showlegend=False,
        autosize=False,
...
def plot_3d_2k(X, pca):
    ...
    Zgrid = np.concatenate(
        (z1grid.flatten()[:, None], z2grid.flatten()[:, None]), axis=1
    )
    Xgrid = pca.inverse_transform(Zgrid)
...
def plot_multicollinearity_3d(X_mc, pca):
    ...
    trace1 = go.Scatter3d(
        ...
        z=X_mc[:, 2],
        mode="markers",
        marker=dict(color="linen", size=5, line=dict(color="black", width=3)),
    )
    trace2 = go.Cone(
        x=[0, 0, 0],
        y=[0, 0, 0],
...
def plot_pca_regression(X, error_type="both", ax=None, title="PCA vs. Regression"):
    ...
    r = [[np.cos(theta)], [np.sin(theta)]] / np.linalg.norm(
    ...
    if np.rad2deg(theta) <= 90:
        ax.plot(
            [np.min((v[:, 0], X[:, 0])), np.max((v[:, 0], X[:, 0]))],
            [m * np.min((v[:, 0], X[:, 0])), m * np.max((v[:, 0], X[:, 0]))],
            color="green",
            label="model",
        )
    else:
    ...
    for i in range(0, X.shape[0]):
        if error_type == "pca" or error_type == "both":
            ax.plot(
                [X[i, 0], v[i, 0]], [X[i, 1], v[i, 1]], color="black", linestyle="-"
            )
        if error_type == "lm" or error_type == "both":
            ax.plot(
                [X[i, 0], X[i, 0]], [X[i, 1], m * X[i, 0]], color="red", linestyle="-."
            )
...
def plot_pca_faces(X_faces, image_shape, index=30):
    ...
    fix, axes = plt.subplots(
        3, 6, figsize=(15, 8), subplot_kw={"xticks": (), "yticks": ()}
    )
    for i, ax in enumerate(axes):
        # plot original image
        ax[0].imshow(X_faces[i + index].reshape(image_shape), vmin=0, vmax=1)
...
def plot_components(W, image_shape):
    fig, axes = plt.subplots(
        2, 5, figsize=(10, 4), subplot_kw={"xticks": (), "yticks": ()}
    )
...
def plot_orig_reconstructed_faces(X, reconstructed_images, image_shape=(200, 200)):
    fig, axes = plt.subplots(
        1, 5, figsize=(10, 6), subplot_kw={"xticks": (), "yticks": ()}
    )
...
def plot_orig_compressed(orig, compressed, n_components):
    fig, ax = plt.subplots(
        1, 2, figsize=[6, 4], subplot_kw={"xticks": (), "yticks": ()}
    )
...
def plot_example_dist(data, centroids, fig, fontsize=16, point_ind=None, ax=None):
    ...
    discrete_scatter(
        centroids[:, 0], centroids[:, 1], y=[0, 1, 2], s=18, markers="*", ax=ax
    )
...
def plot_km_iteration(X, Z, centers, new_centers, iteration, fig, ax, fontsize=18):
    ...
    ax[0].set_title(
        f"Iteration: {iteration}: Update Z", fontdict={"fontsize": fontsize}
    )
    discrete_scatter(
        X[:, 0], X[:, 1], y=Z.tolist(), markers="o", s=12, label_points=True, ax=ax[1]
    )
    discrete_scatter(
        new_centers[:, 0],
        new_centers[:, 1],
    ...
    ax[1].set_title(
        f"Iteration: {iteration}: Update cluster centers",
        fontdict={"fontsize": fontsize},
    )
    plt.close()
    return fig
...
def plot_km_iterative(X, starting_centroid, iterations=5, k=3):
    ...
    discrete_scatter(
        centroids[:, 0], centroids[:, 1], y=[0, 1, 2], markers="*", s=16, ax=ax[0]
    )
...
def Gaussian_mixture_1d(ϕ1, ϕ2, fig, μ1=0.0, μ2=5.0, Σ1=1, Σ2=3):
    ...
    data = pd.DataFrame(
        {
            "x": np.arange(
                np.min([μ1 - 4 * Σ1, μ2 - 4 * Σ2]),
                np.max([μ1 + 4 * Σ1, μ2 + 4 * Σ2]),
                1 / 1000,
            )
        }
    )
    ...
    plt.plot(
        ...
        data=data,
        color="green",
        label=f"$f_1(x|\mu_1={μ1},\Sigma_1={Σ1})$",
    )
    plt.plot(
        "x",
        "f2(x|mu2,Sigma2)",
        ...
        data=data,
        color="red",
        label=f"$f_2(x|\mu_2={μ2},\Sigma_2={Σ2})$",
    )
    plt.legend(fontsize=16)
    plt.title(
        f"Gaussian Mixture: $\Phi_1$ = {ϕ1} and $\Phi_2$ = {ϕ2}",
...
def plot_kmeans_circles(kmeans, X, n_clusters=3, ax=None):
    ...
    discrete_scatter(centers[:, 0], centers[:, 1], y=[0, 1, 2], markers="*", s=18)
    radii = [
        cdist(X[km_labels == i], [center]).max() for i, center in enumerate(centers)
    ]
...
def plot_cov_types(
    X_train, gmm_full_labels, gmm_tied_labels, gmm_diag_labels, gmm_spherical_labels
):
    fig, ax = plt.subplots(2, 2, figsize=(12, 8))
    discrete_scatter(
        X_train[:, 0],
        X_train[:, 1],
        gmm_spherical_labels,
        c=gmm_spherical_labels,
        markers="o",
        ax=ax[0][0],
    )
...
def get_cluster_images(model, Z, inputs, cluster=0, n_img=5):
    ...
    if type(model).__name__ == "GaussianMixture":
        ...
        axes[0].imshow(
            np.transpose(inputs[inds[0]].reshape(img_shape) / 2 + 0.5, transpose_axes)
        )
    ...
    for i, min_samples in enumerate([2, 3, 5]):
        for j, eps in enumerate([1, 1.5, 2, 3]):
            ...
            if np.any(clusters == -1):
                ...
                m = markers
            discrete_scatter(
                X[:, 0], X[:, 1], clusters, ax=axes[i, j], s=5, c=c, markers=m
            )
...
def plot_X_dendrogram(
    X, linkage_array, font_size=14, label_n_clusters=False, title="Dendrogram"
):
    ...
    if label_n_clusters:
        ...
        axes[1].text(
            bounds[1], 7.25, " two clusters", va="center", fontdict={"size": 15}
        )
        axes[1].text(
            bounds[1], 4, " three clusters", va="center", fontdict={"size": 15}
        )
...
def print_hierarchical_clusters(
    inputs,
    Z,
    cluster_labels,
    unique_cluster_labels=[2, 3, 7, 8, 13, 15, 17, 19, 20, 21, 22, 27, 29, 30],
):
    transpose_axes = (1, 2, 0)
    for cluster in unique_cluster_labels:  # hand-picked "interesting" clusters
        ...
        for image, label, ax in zip(inputs[mask], cluster_labels[mask], axes):
            ax.imshow(np.transpose(image.numpy() / 2 + 0.5, transpose_axes))
...
def plot_dendrogram_clusters(
    X, linkage_array, hier_labels, linkage_type="single", title=None
):
    fig, ax = plt.subplots(1, 2, figsize=(12, 4))
    ...
    discrete_scatter(
        X[:, 0], X[:, 1], hier_labels, markers="o", label_points=True, ax=ax[1]
    )
...
def my_heatmap(
    ...
    for p, color, value in zip(
        img.get_paths(), img.get_facecolors(), img.get_array().flatten()
    ):
...
def plot_loss_diagram(
    labels_inside=False,
):  # From Mike's notebook: https://github.com/UBC-CS/cpsc340-2020w2/blob/main/lectures/19_linear-classifiers-fit.ipynb
...
def plot_svc_gamma(
    param_grid, X_train, y_train, x_label="longitude", y_label="latitude"
):
...
def plot_confusion_matrix_ex(tn, fp, fn, tp, target="Fraud"):
    ...
    plt.xticks(
        [0.25, 0.75],
        ["predicted not " + target, "predicted " + target],
        size=20,
        rotation=25,
    )
...
def plot_confusion_matrix_example(tn, fp, fn, tp, target="Fraud"):
    fig, ax = plt.subplots(
        1, 2, figsize=(20, 6), subplot_kw={"xticks": (), "yticks": ()}
    )
...
def plot_multiclass_lr_ovr(
    lr, X_train, y_train, n_classes, test_points=None, decision_boundary=False
):
